...
module ColTs {
    ...
    export interface CollectionBase<E> {
        unique<T>():Col<E>;
        clone<E>():Col<E>;
        length():number;
    ...
    export class Col<E> implements CollectionBase<E> {
        ...
        groupBy(keySelector:(item:E)=>string = (e) => e.toString()):Map<E[]> {
        ...
        toMap<R>(keySelector:(item:E)=>string, valueSelector:(p1:E)=>R):Map<R> {
            var keyedCol = this.select(x => {
                return {key: keySelector(x), value: valueSelector(x)};
            });

            return new Map<R>(keyedCol.toArray());
        }
    }
}
