...
import Col = ColTs.Col;
import Map = ColTs.Map;


class ExamplesWithColTS {
    ...
    static findMostFiveCommonWordAndTheirOccurrences(text:string) {
        ...
        return Col.of(cleanText.split(" "))
            ...
            .orderByDesc(e => e.count).take(5).toArray();
    }

    static countfavoriteFruitsFromPeopleAbove30(data:GeneratedData[]) {
        return Col.of(data)
            .where(e => e.age > 30)
            .groupBy(e => e.favoriteFruit)
            .selectValues(e => e.value.length)
            .toHashmap(key => key, val => val);
    }


    static avarageBalancePerDecade(data:GeneratedData[]) {
        return Col.of(data)
            .groupBy((e) => Math.floor(e.age / 10).toString())
            .toHashmap(
                e => e + "0s",
                arr => Col.of(arr).average(
                    e => parseFloat(e.balance.replace(/[^0-9\.]/g, ""))
            )
        );
    }
